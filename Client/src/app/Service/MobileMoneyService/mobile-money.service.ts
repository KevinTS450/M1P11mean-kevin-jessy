...
export class MobileMoneyService {
  ...
  public updateMobileMoney(mobileMoney:MobileMoney): Observable<any>{
    return this.http.put(`${this.baseUrl}`+`/updateById`+`/${mobileMoney._id}`, mobileMoney);
  }

  public getMyMobileMoney(user:User): Observable<MobileMoney> {
    return this.http.get<MobileMoney>(`${this.baseUrl}/findByUser`+`/${user._id}`+`/${user.name}`+`/${user.email}`);
  }
}
